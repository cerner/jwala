...
<Server port="${jvm.shutdownPort}" shutdown="SHUTDOWN">
  ...
  <Listener className="com.cerner.jwala.tomcat.listener.messaging.jgroups.JGroupsReportingLifeCycleListener"
            ...
            ${vars['jgroups.coordinator.ip.address'] == null ? "jgroupsCoordinatorHostname=" + /"/ + vars['jgroups.coordinator.hostname'] + /"/  : "jgroupsCoordinatorIp=" + /"/ + vars['jgroups.coordinator.ip.address'] + /"/}
  ...
  <Service name="stp">
    <!-- Define a SSL HTTP/1.1 Connector.  This connector uses the JSSE configuration, when using APR, the
        ...
        ServerXMLTemplate.erb
    -->

    <Connector 
        port="${jvm.httpsPort}"
        SSLCertificateFile="D:/ctp/app/data/security/id/${jvm.hostName.tokenize('.')[0].toLowerCase()}.cer"
	SSLCertificateKeyFile="D:/ctp/app/data/security/id/${jvm.hostName.tokenize('.')[0].toLowerCase()}.key"
  ...
  <Service name="Catalina">
    ...
    -->
    <Engine name="Catalina" defaultHost="localhost" jvmRoute="localhost">
